...
class AngleKDRotatedRetinaHead(RotatedRetinaHead):
    ...
    def __init__(self,
                 ...
                 loss_alp_weight2=dict(
                     ...
                     T=2),
                 **kwargs):
        super(AngleKDRotatedRetinaHead, self).__init__(
            num_classes,
            in_channels,
            **kwargs)
        self.loss_alp_weight1 = build_loss(loss_alp_weight1)
        self.loss_alp_weight2 = build_loss(loss_alp_weight2)
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        soft_targets = soft_targets.permute(0, 2, 3, 1).reshape(-1, 5)
        ...
        # regression loss
        bbox_pred = bbox_pred.permute(0, 2, 3, 1).reshape(-1, 5) # [2304, 45], 2304= 81*5*16*16/(9*5)=2304
        bbox_targets = bbox_targets.reshape(-1, 5)
        bbox_weights = bbox_weights.reshape(-1, 5)
        anchors = anchors.reshape(-1, 5)
        if self.reg_decoded_bbox:
            bbox_pred = self.bbox_coder.decode(anchors, bbox_pred)
            soft_targets = self.bbox_coder.decode(anchors, soft_targets)
        ...
        bbox_pred = self.bbox_coder.decode(anchors, bbox_pred)
        soft_targets = self.bbox_coder.decode(anchors, soft_targets)
        loss_alp_weight1 = self.loss_alp_weight1(
            bbox_pred, soft_targets, weight=ld_region[:, None].expand(-1, 5), avg_factor=ld_num)
        
        loss_alp_weight2 = self.loss_alp_weight2(cls_score, soft_labels, weight=ld_region, avg_factor=ld_num)
        return loss_cls, loss_bbox, loss_alp_weight1, loss_alp_weight2
    ...
    def loss(self,
        ...
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
         num_total_pos, num_total_neg, ld_region_list) = cls_reg_targets
        num_total_samples = (
        ...
        losses_cls, losses_bbox, losses_alp_weight1, losses_alp_weight2 = multi_apply(
            self.loss_single,
            cls_scores,
            bbox_preds,
            all_anchor_list,
        ...
        #     loss_alp_weight1=losses_alp_weight1,
        #     loss_alp_weight2=losses_alp_weight2)
        return dict(
            loss_cls=losses_cls,
            loss_bbox=losses_bbox,
            loss_alp_weight1=losse s_alp_weight1,
            loss_alp_weight2=losses_alp_weight2)
    ...
    def _get_targets_single(self,
        ...
        return (labels, label_weights, bbox_targets, bbox_weights, pos_inds,
                neg_inds, sampling_result, ld_region)
    ...
    def get_targets(self,
        ...
        (all_labels, all_label_weights, all_bbox_targets, all_bbox_weights,
         pos_inds_list, neg_inds_list, sampling_results_list, all_ld_region) = results[:8]
        ...
        res = (labels_list, label_weights_list, bbox_targets_list,
               bbox_weights_list, num_total_pos, num_total_neg, ld_regions_list)
